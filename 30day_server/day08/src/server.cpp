...
void Server::NewConnenction(Socket *serv_sock) {
    ...
    std::function<void(Socket *)> callback = [this](auto &&PH1) { DeleteConnection(std::forward<decltype(PH1)>(PH1)); };
    conn->set_delete_connection_callback_(callback);
    connections_[serv_sock->get_fd()] = conn;
}

void Server::DeleteConnection(Socket *sock) {
    Connection *conn = connections_[sock->get_fd()];
